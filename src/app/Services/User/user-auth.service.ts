...
export class UserAuthService {
  ...
  login(log: logDTO): Observable<any> {
    ...
    // );
     return this.http.post<any>('http://localhost:5248/api/Account/login', log, {
      withCredentials: true,
      observe: 'response'
    }).pipe(
  ...
  logout(): Observable<any> {
    return this.http.post<any>('http://localhost:5248/api/Account/logout', null, {
      withCredentials: true,
      observe: 'response'
    }).pipe(
  ...
  externallogin(provider: string, returnUrl: string) {
    const url = `http://localhost:5248/api/Account/External-login?provider=${encodeURIComponent(provider)}&returnUrl=${encodeURIComponent(returnUrl)}`;
